...
const mapOnboarding = (raw: any): OnboardingState => {
  ...
  if (raw.is_complete !== undefined) return raw;
  const { notion_connected, journal_db_selected, tasks_db_selected,
          template_chosen, schedule_set, completed_at } = raw;
  // completed_at being set ALWAYS means onboarding is done — trust the server
  if (completed_at) return { step: 'complete', is_complete: true };
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const setScheduleTime = async (time: string, timezone: string) => {
    try {
      ...
      // Force onboarding complete IMMEDIATELY in local state
      // Do NOT wait for the server refresh — this prevents the bounce-back loop
      setOnboarding({ step: 'complete', is_complete: true });

      addToast('Schedule set! Your first journal generates tomorrow morning.', 'success');

      // Navigate then confirm with server in background
      navigate('/dashboard');

      // Background refresh to sync any server state
      authApi.me().then(({ data: meData }) => {
        if (meData?.onboarding) {
          const mapped = mapOnboarding(meData.onboarding);
          // Only update if server also says complete — never override local complete with incomplete
          if (mapped.is_complete) setOnboarding(mapped);
        }
      ...
      if (err.response?.data?.code === 'PLAN_REQUIRED') {
        setOnboarding({ step: 'complete', is_complete: true });
        addToast("You're all set! Generate your first journal manually.", 'success');
        navigate('/dashboard');
        return;
      }
      addToast(msg, 'error');
  ...
  const generateJournalNow = async () => {
    try {
      ...
      if (code === 'FREE_LIMIT_REACHED') {
        addToast(
          "You've used all 5 free generations this month. Upgrade to Pro for unlimited journals.",
          'error'
        );
        return;
      }
